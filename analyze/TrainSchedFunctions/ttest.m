...
% 05/06 changed by Sebastian: can now take in a constant when doing
% onesample ttests. The constant must be supplied in place of groupB
...
switch (kind)
    ...
    case 'onesample'
        if isempty(groupB) || length(groupB)>1
            constant=0;
        else
            constant=groupB;
        end
        ...
        t=(mean(groupA(indx))-constant)/SE;
        if tails==2
            p=2*(1-tcdf(abs(t),df));
        else 
            p=1-tcdf(t,df);
